...
const template = fs.readFileSync(path.join(__dirname, "..", "prompts", "cv.txt"), "utf8");
...
for (let i = 0; i < eligible.length; i++) {
  ...
  const prompt = template
    ...
    .replace("{{TARGET_ROLE}}", job.cv_focus || job.job_title || job.title || "Software Engineer")
    .replace("{{JOB_DESCRIPTION}}", JSON.stringify(job, null, 2));

  const cv = await callGeminiWithRetry(prompt);
  if (!cv) {
  ...
  const baseName = `${job.company.replace(/[^a-z0-9]/gi, "_").toLowerCase()}_${job.title
    ...
    .toLowerCase()
    .slice(0, 20)}`;
  fs.writeFileSync(`generated/${baseName}-cv.md`, cv);
  generated++;
  console.log(`  ✅ ${label}`);
